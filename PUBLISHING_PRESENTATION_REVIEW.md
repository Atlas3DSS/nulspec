...
| WPR-LOCAL-003 | Research/web handoff | The first push named this checkout's legacy `origin`, which points to the older reproduction repository rather than the configured `nulspec` remote. | The intended commit remained local and PR #21 was unchanged; an unintended new branch containing only this documentation commit appeared in the older repository. | Resolve and push the exact branch to the explicit `nulspec` remote, verify PR #21's head, then remove only the accidental remote branch after confirming the commit is preserved. |
| WPR-LOCAL-004 | Research/web handoff | The first PR #21 verification query requested the unsupported `headRefOid` field from the installed older GitHub CLI. | The read-only query failed without changing either repository or pull request. | Use the supported `commits` and branch fields, then compare the final commit through Git's remote ref directly. |
| WPR-LOCAL-005 | Research/web handoff | The next compact PR-status query assumed `statusCheckRollup` was an array, but this documentation-only PR returned `null`. | The read-only query failed during local projection; GitHub still returned the underlying PR metadata and no state changed. | Read the supported JSON without an array projection, verified PR #21's exact commit list, and compared both remote refs before removing the accidental branch. |
