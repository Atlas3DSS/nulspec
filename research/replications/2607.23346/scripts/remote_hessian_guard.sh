#!/usr/bin/env bash
# Fail-closed guard for exploratory Hessian processes on the shared dev box.
...
OUTPUT_ROOT="$BASE/outputs/hessian-extensions"
...
validated_pid() {
    ...
    [[ "$command" == *"$BASE/source/run_hessian_extensions.py"* ]] || return 1
...
terminate_seed() {
    ...
    log "stopping Hessian seed $seed pid=$pid: $reason"
...
log "Hessian guard started; floor=8GiB ceiling=88C"
while ! all_complete_or_stopped; do
    if ! pgrep -f -- "$PROTECTED_PROCESS_PATTERN" >/dev/null; then
        ...
        log "Hessian guard tripped on protected-process absence"
        exit 86
    fi
    ...
    if (( available_kib < MIN_AVAILABLE_KIB )); then
        ...
        log "Hessian guard tripped on host memory"
        exit 87
    fi
    ...
    if (( temp_primary >= MAX_TEMP_C )); then
        ...
        log "Hessian guard tripped on primary GPU temperature"
        exit 88
    fi
    if (( temp_secondary >= MAX_TEMP_C )); then
        ...
        log "Hessian guard tripped on secondary GPU temperature"
        exit 89
    fi
    sleep 10
done
log "Hessian guard complete; no active incomplete Hessian process remains"
