...
def test_hash_artifact_tree_records_exact_base_input(tmp_path: Path) -> None:
    ...
    assert (
        records["final_model/model.safetensors"]["sha256"]
...
def test_primary_runner_hashes_base_before_factorization() -> None:
    ...
    trap_position = source.index("trap finish_attempt EXIT")
    environment_position = source.index("capture_python_environment.py")
    ...
    assert trap_position < environment_position
